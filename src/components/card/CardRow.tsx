...
function CardRow({
  ...
  return variation === cardRowVariations.DETAIL ? (
    <div className={`${styles.detailRow} flex items-center justify-between h-20 mx-10 border-b border-dashed border-sold-middle-gray`}>
      <div className="w-1/2 text-sold-dark-gray">{label}</div>
      <h1 className={`w-1/2 text-right text-2xl font-semibold ${fontColor}`}>{value}</h1>
    </div>
  ) : (
    <div className={`${styles.totalRow} flex flex-col justify-center items-end h-20 px-10 bg-sold-semilight-gray`}>
